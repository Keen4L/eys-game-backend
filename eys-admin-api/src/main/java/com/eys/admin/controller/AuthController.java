...
import com.eys.model.dto.auth.AdminLoginDTO;
 ...
 * @author EYS
 */
@Tag(name = "认证管理", description = "管理员登录/登出接口")
@RestController
@RequestMapping("/auth")
...
public class AuthController {
     ...
     * 管理员登录
     */
    @Operation(summary = "管理员登录", description = "使用账号密码登录管理后台")
    @PostMapping("/login")
    public R<LoginVO> login(@Valid @RequestBody AdminLoginDTO dto) {
        LoginVO loginVO = sysUserService.adminLogin(dto.getUsername(), dto.getPassword());
        return R.ok(loginVO);
    }
